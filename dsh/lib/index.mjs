...
*/function renderWriteReceipt(receipt){const outcome=receipt.outcome??{};const written=outcome.written??[];const superseded=outcome.superseded??[];const rejected=outcome.rejected??[];const reinforced=outcome.reinforced??[];const retracted=outcome.retracted??[];const purged=outcome.purged??[];if(receipt.status==="pending")return"已入队（尚未落库）。稍后可用 memory_snapshot 或 memory_summary_detail 确认结果。";if(receipt.status==="error")return`写入失败：${receipt.reject_reason??"存储侧报错"}（未写入任何记忆）`;if(purged.length>0)return`已彻底删除 ${purged.length} 条记忆（不可恢复）。`;if(retracted.length>0)return`已遗忘 ${retracted.length} 条记忆（软删除）。`;const parts=[];if(written.length>0)parts.push(`已记住 ${written.length} 条`);if(superseded.length>0){const detail=superseded.map(s=>`${s.predicate??"?"}: ${s.old_object??"?"} → ${s.new_object??"(替换)"}`).join("；");parts.push(`并替换了 ${superseded.length} 条旧值（${detail}）`);}if(reinforced.length>0)parts.push(`其中 ${reinforced.length} 条已存在，记为复用确认`);if(rejected.length>0){const first=rejected[0];const reason=first.detail||first.reason||(first.kind==="conflict"?"与已存记忆冲突":String(first.kind??"被拒绝"));parts.push(`拒绝 ${rejected.length} 条：${reason}`);}if(parts.length===0)return"没有可写入的事实（抽取为空）。";return`${parts.join("，")}。`;}/** Ask the store for the outcome of a write, within a bounded wait. */function writeParams(deps,extra){return{wait_ms:deps.writeAckTimeoutMs??0,...extra};}/** Thrown when the memory master switch is off. */function disabledError(){return/* @__PURE__ */new Error("memory is disabled");}/** Register all memory tools and return their disposers. */function registerMemoryTools(deps){const{ctx,bridge}=deps;const disposers=[];const scope=deps.fallbackScope;const call=(method,params)=>bridge.call(method,params);const budget=()=>deps.resolveSummaryBudget?.()??deps.summaryTokens;disposers.push(ctx.tools.register(defineTool({name:"memory_add",description:"显式记住一条用户偏好、事实、事件、流程图或经验教训。传入原始内容，系统会自行抽取为原子事实；若与已存记忆冲突，系统会按证据强度决定替换或拒绝，并在结果里说明。",parameters:{content:{type:"string",required:true,description:"要记住的原始内容"},user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:renderWriteReceipt(value)}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);const sid=sessionIdOf(exec,scope);const raw=args.content;if(deps.extract!==void 0)try{const candidates=await deps.extract(raw);if(candidates.length>0){const r=await call("persist_candidates",writeParams(deps,{user_id:uid,session_id:sid,turn_id:0,candidates:stampExplicitPriority(candidates)}));return{...r,candidate_id:r.candidate_id??""};}}catch{}const body=raw.trim();if(body.length>=RAW_KNOWLEDGE_MIN_CHARS){const r=await call("persist_candidates",writeParams(deps,{user_id:uid,session_id:sid,turn_id:0,candidates:[rawKnowledgeCandidate(body)]}));return{...r,candidate_id:r.candidate_id??"",fallback:"raw"};}return await call("add",writeParams(deps,{user_id:uid,session_id:sid,text:raw,turn_id:0}));}})));disposers.push(ctx.tools.register(defineTool({name:"memory_replace",description:"用新内容替换一条已有记忆（按 fact_id 指名替换）。用于纠正写错的记忆：新内容会被抽取为事实，被指名的那条随之退役（superseded）。",parameters:{factId:{type:"string",required:true,description:"要被替换的记忆 fact id（可用 memory_summary_detail 获取）"},content:{type:"string",required:true,description:"替换后的新内容"},user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:renderWriteReceipt(value)}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();if(!args.factId)throw new Error("memory_replace requires factId");if(!args.content)throw new Error("memory_replace requires content");return await call("replace",writeParams(deps,{user_id:args.user??userIdOf(exec,scope),fact_id:args.factId,new_text:args.content}));}})));disposers.push(ctx.tools.register(defineTool({name:"memory_recall",description:"检索与查询相关的持久记忆原子事实。查询用名词短语/关键词效果最好；若返回空，说明记忆库里没有足够相关的条目（而不是系统故障）。",parameters:{query:{type:"string",required:true,description:"要检索的记忆查询"},user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"},topK:{type:"integer",description:"返回条数上限（默认按配置）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){const v=value;const facts=v.facts??[];const blocks=[];if(facts.length===0)blocks.push("（无相关记忆）");else blocks.push(facts.map(f=>{const head=[f.fact_id?`[${f.fact_id}]`:"",`${f.subject??""}${f.predicate??""}: ${f.object??""}`,f.type?`*(${f.type})*`:""].filter(Boolean).join(" ");const body=(f.content??"").trim();return body?`- ${head}\n    > ${body}`:`- ${head}`;}).join("\n"));if((v.degraded??[]).length>0)blocks.push(`（注意：检索索引 ${(v.degraded??[]).join(" / ")} 本次不可用，结果可能不完整）`);return[{type:"text",text:blocks.join("\n")}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);const r=await call("recall",{user_id:uid,query:args.query,token_budget:4e3,top_k:args.topK??deps.maxRecalledFacts});return{facts:r.facts??[],token_count:r.token_count??0,degraded:r.degraded??[]};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_summary",description:"渲染当前用户记忆的紧凑摘要（与注入系统提示词的快照同一预算、同一份渲染，但不含数据围栏）。适合先看摘要，再按需用 memory_recall 查明细；要确认提示词里真正冻结的那段，用 memory_snapshot；要定位/编辑具体某条事实，用 memory_summary_detail。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("summary",{user_id:uid,max_tokens:budget(),detail:false})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_snapshot",description:"返回当前会话系统提示词中真正冻结的那段记忆快照（含数据围栏原样）。用于核对模型实际看到了什么；若本会话尚未冻结，会即时冻结并返回同一份文本。",parameters:{},output:{schema:{type:"object",additionalProperties:true},render(_args,value){const v=value;return[{type:"text",text:`${v.frozen===false?"（尚未冻结：以下为即时渲染，开新会话将按此冻结）\n":""}${v.text??"（无）"}`}];}},async execute(_args,exec){if(deps.isEnabled?.()===false)throw disabledError();if(deps.snapshot===void 0)return{text:"（本部署未启用快照注入）",frozen:false};const sid=sessionIdOf(exec,scope);const existing=deps.snapshot.peek(sid);const text=existing??(await deps.snapshot.ensure(sid));if(!text)return{text:"（无内容：记忆库为空，或注入已关闭）",frozen:existing!==void 0};return{text,frozen:existing!==void 0};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_summary_detail",description:"渲染当前用户记忆的完整清单（每条含 fact_id，便于定位与编辑）。注入系统提示词的是紧凑版（按类型分组、不含 fact_id）——如需确认注入内容，用 memory_snapshot。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("summary",{user_id:uid,max_tokens:deps.summaryTokens,detail:true})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_forget",description:"删除一条记忆。默认软删除（retract，可被后续 backp 恢复）；purge=true 时彻底删除（含索引与复用证据，不可恢复）。",parameters:{factId:{type:"string",description:"记忆 fact id（二选一）"},purge:{type:"boolean",description:"是否彻底删除（默认 false：软删除）"},user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:renderWriteReceipt(value)}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();if(!args.factId)throw new Error("memory_forget requires factId");return await call("forget",writeParams(deps,{user_id:args.user??userIdOf(exec,scope),fact_id:args.factId,purge:args.purge===true}));}})));disposers.push(ctx.tools.register(defineTool({name:"memory_user_md",description:"渲染当前用户的画像卡片 markdown（画像由活跃事实即时投影而来）。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){return[{type:"text",text:value.text??""}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();const uid=args.user??userIdOf(exec,scope);return{text:await call("user_md",{user_id:uid})};}})));disposers.push(ctx.tools.register(defineTool({name:"memory_stats",description:"返回当前用户的记忆统计计数，以及最近几次写入的结果（含被拒绝的原因）。",parameters:{user:{type:"string",description:"可选：归属用户 id（默认当前用户，跨会话共享）"}},output:{schema:{type:"object",additionalProperties:true},render(_args,value){const v=value;const lines=[`活跃记忆 ${v.facts??0} 条 · 待处理 ${v.pending??0} 条 · 归档 ${v.archived??0} 条`];for(const entry of v.recent??[])if(entry.reject_kind)lines.push(`- 最近一次写入被拒绝（${entry.reject_kind}）：${entry.reject_reason??""}`);return[{type:"text",text:lines.join("\n")}];}},async execute(args,exec){if(deps.isEnabled?.()===false)throw disabledError();return await call("stats",{user_id:args.user??userIdOf(exec,scope)});}})));return disposers;}/**
...
*/function registerMemoryContext(deps){const{ctx,bridge,userScope}=deps;ctx.systemPrompt.section({name:AWARENESS_SECTION,order:ctx.systemPrompt.getSectionOrder("TOOL_SESSION_QUERY"),text:()=>deps.isEnabled?.()===false?"":AWARENESS_TEXT});const maxFrozen=deps.maxFrozenSessions??200;/** sessionId -> frozen injected text (insertion order == recency). */const frozen=/* @__PURE__ */new Map();/**
	...
	*/const snapshotFor=async sessionId=>{const cached=frozen.get(sessionId);if(cached!==void 0)return cached;let rendered;try{const raw=await bridge.call("summary",{user_id:userScope,max_tokens:deps.resolveMaxTokens(),detail:false,include_meta:true});const text=typeof raw==="string"?raw:raw?.text??"";if((typeof raw==="string"?void 0:raw?.facts)===0){frozen.set(sessionId,"");return"";}rendered=renderMemoryDataBlock((text??"").trim());}catch{return"";}if(!rendered)return"";if(frozen.size>=maxFrozen){const oldest=frozen.keys().next().value;if(oldest!==void 0)frozen.delete(oldest);}frozen.set(sessionId,rendered);return rendered;};/** Insert the snapshot right after the awareness section (else append). */const injectSection=(assembly,text)=>{if(assembly.sections.some(s=>s.name===SNAPSHOT_SECTION))return;const section={name:SNAPSHOT_SECTION,text};const anchor=assembly.sections.findIndex(s=>s.name===AWARENESS_SECTION);if(anchor>=0)assembly.sections.splice(anchor+1,0,section);else assembly.sections.push(section);};ctx.on("system-prompt/assemble",async(_assembly,context,next)=>{const assembly=await next();if(deps.isEnabled?.()===false)return assembly;if(deps.snapshotEnabled()===false)return assembly;const sessionId=context.agent?.session?.id;if(sessionId===void 0)return assembly;const text=await snapshotFor(sessionId);if(text)injectSection(assembly,text);return assembly;});return{peek:sessionId=>frozen.get(sessionId),ensure:async sessionId=>{if(deps.snapshotEnabled()===false||deps.isEnabled?.()===false)return"";return await snapshotFor(sessionId);}};}//#endregion
...
* Python side.
*/var AtomMemoryController=class AtomMemoryController extends TypertRemoteService{static{[_initProto]=_applyDecs(this,[],[[Remote,2,"health"],[Remote,2,"listFacts"],[Remote,2,"editFact"],[Remote,2,"deleteFact"],[Remote,2,"summary"],[Remote,2,"unarchive"],[Remote,2,"listProfile"],[Remote,2,"upsertProfile"],[Remote,2,"deleteProfile"],[Remote,2,"backup"],[Remote,2,"restore"],[Remote,2,"getRuntime"]],0,void 0,TypertRemoteService).e;}bridge=void _initProto(this);runtime;startupError;constructor(ctx,bridge,runtime,startupError=()=>void 0){super(ctx,"atomMemoryController",{namespace:"atomMemory"});this.bridge=bridge;this.runtime=runtime;this.startupError=startupError;}/** Whether the bridge is alive and the plugin master switch is on. */assertReady(){if(!this.runtime.isEnabled())throw new Error("memory is disabled");if(!this.bridge.alive){const reason=this.startupError();throw new Error(reason!==void 0?`memory bridge is not running: ${reason}`:"memory bridge is not running");}}/**
...
*/const inject=["tools","systemPrompt"];/** Fallback user/session scope for a single-user local harness. */const FALLBACK_SCOPE="global";/** Start attempts before the bridge is declared offline. */const MAX_START_ATTEMPTS=3;/**
