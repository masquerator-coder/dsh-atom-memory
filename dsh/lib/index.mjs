...
*/const REFUSAL_LEADS={target_not_active:"未替换：目标事实不存在或已不是活跃状态",extraction_empty:"未替换：新内容没能抽取成任何事实"};/** Fallback wording when the store named a kind but sent no reason. */const REFUSAL_HINTS={target_not_active:"它可能已被替换、遗忘，或 fact_id 属于其他用户",extraction_empty:"replace 没有原样兜底，请改用 memory_add 记录自由文本，再决定是否遗忘原条目"};/**
