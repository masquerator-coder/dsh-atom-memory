...
用 `listFacts({limit: 200})` 取数——**第 200 条之后的记忆根本取不到**（`list_facts`
自身把 `limit` 钳到 `[1, 200]`，`atom_memory/api.py:1532`），同时它还把返回信封里
的 `total` 丢掉，页面上没有任何地方能看出总共有多少条。

现在：

- **分页是真·服务端分页**：翻页调用 `listFacts({offset, limit})`，由 `list_facts`
  的 `LIMIT/OFFSET` 出页面，`total` 在同一次往返里数全表。不是把全量取回来再在前端
  切片——那正好是「条数大」时不该做的事。
- **每页条数可选**：20 / 50 / 100 / 200（默认 50），上限对齐 store 自己的钳制值，
  不提供做不到的选项。换页长时回到第 1 页，因为同一个页码在新页长下指向另一批行。
- **总数显示在两处**：设置页「编辑记忆」按钮旁显示 `共 N 条`（打开弹窗前就能看到），
  弹窗内显示 `第 X-Y 条 / 共 N 条` 与 `第 P/共 pages 页`。
- 计数取**信封里的 `total`**，不是 `facts.length`——后者在分页后就是页长；`total`
  缺失或不可解析时退化为页长，避免渲染出 `NaN`（会被读成「0 条记忆」）。

**翻页即替换草稿行，不做跨页草稿。** 保存提交的就是屏幕上的那一页，`saveAllFacts`
的 diff 也取自同一页快照。这是有意的：把改了一半的行留在跨页状态里，会让一个陈旧
草稿被写回用户已经翻走、看不见的那一行。末页删除导致 `total` 变小到当前页不存在时，
自动退回最后一页；翻页失败时保留原页面并显示失败原因，而不是把表格清空。

`dsh/src/client/{memory-settings-controller.ts,MemorySettingsSection.tsx,locales.ts,styles.ts}`

### Investigated (规模扫描：拐点在 5k，且当前依赖没有 ANN 可用)

用 `bench/scale_scan.py` 在合成库（512 维真实随机向量、与生产同一个
`_vector_knn`、同 scope 过滤、pool=40）上扫描精确 KNN 的规模曲线：

| facts | `_vector_knn` p50 | raw vec0（无过滤） | FTS p50 |
|---|---|---|---|
| 1,000 | 3.36 ms | 1.53 ms | 0.01 ms |
| 2,500 | 8.39 ms | 3.73 ms | 0.01 ms |
| 5,000 | 16.20 ms | 6.67 ms | 0.01 ms |
| 10,000 | 54.74 ms | 22.46 ms | 0.06 ms |
| 20,000 | **110.67 ms** | 46.54 ms | 0.03 ms |

**每千条成本在 1k–5k 稳定在 3.2–3.4 ms，10k 跳到 5.47 ms**——拐点在 5k 之后
（约 12 MB 库），20k 是 20 倍规模、33 倍耗时。原先设想的「>5000 条才需要处理」
低估了：5k 时已 16.2 ms，而当前生产库（1061 条）是 9 ms。

**更关键的是：这个依赖根本没有 ANN 可用。** 实测
`vec0(..., index=hnsw)` 与 `vec0(..., partitions=16)` 都被拒绝
（`Unknown table option`），pin 的 sqlite-vec v0.1.9 不含任何 ANN 索引，
也不导出建索引函数。「换 ANN」在这个依赖上不是可执行的方案。

唯一可用的杠杆是二值量化粗筛，实测**不可行**：粗筛确实快（20k 时 7.15 ms
vs 精确 45.26 ms），但 recall@40 在深度 100/400/1000/4000 时只有
32.5%/45.0%/72.5%/92.5%，且重排把收益全吃掉（深度 1000 总成本 ~186 ms，
是精确基线的 4 倍）。

根因是 **blob I/O**：重排要读回 2048 字节/条的 float32，20k 条读 1000 个候选
（2 MB）约 99 ms。证据是两种完全不同的距离实现耗时几乎相同
（`vec_distance_cosine` 170 ms vs Python 手写余弦 193 ms）——贵的不是算距离。
这也解释了 P1 里 `IN` 过滤那 85 ms 的额外开销，以及为什么「取深再筛」会崩：
每种变形都在重复读同一批 blob。

**结论：不做 P3 的工程改造。** 唯一加速手段实测更慢且丢召回，唯一有效方向
（真 ANN）无 API。该做的是把规模告警线定在 3,000–5,000 条（而非 5,000 条才
「启动优化」），并在事实数 > 3,000 时启动**依赖选型**调研——那是依赖决策，
不是读路径优化。

### Fixed (vec0 的 k 上限会静默删掉整个语义检索)

`sqlite-vec` 对 KNN 的 `k` 有硬上限，超过时**报错而非钳制**。实测阈值精确在
**4097**（`k=4096` 正常，`k=4097` 抛 `k value in knn query too large`）。
这个错误被 `_vector_knn` 的 `except sqlite3.Error` 吞掉、**返回 `[]`**，
只在 `last_degraded` 里留一个 `vector`——于是过大的 pool 会**静默删掉语义检索
这一半**，召回退化成纯 FTS：仍然能回答，只是更差，而 host 侧不读 `degraded`，
没有任何可见症状。

而 pool = `top_k * candidate_pool_multiplier`，**纯靠配置即可触发**
（`top_k=2000` + 出厂倍数 4 = 8000）。默认配置 40 远低于上限，所以此前未暴露。

修法是钳制而非报错：新增 `retriever.VEC_MAX_K = 4096`，在 `_vector_knn` 内
钳制并记 warning。要求比引擎能排序的还多的邻居是部署配置错误，返回引擎支持的
最深排序比返回空更接近调用方本意。同时 `k <= 0` 直接返回空
（`LIMIT 0` 是白跑一趟，负值会让驱动报错，两者都不是查询）。

实测 `k=4097`/`k=5000` 从「返回 0 条」变为「返回 1087 条」。补 3 个测试，
并做双向验证：去掉钳制后 `test_vector_knn_clamps_a_pool_above_the_engine_ceiling`
失败，报错正是它要防的那条 `k value in knn query too large`。

### Reverted (KNN 成员过滤改造：实测证明是回退，已放弃)

曾尝试把 vec0 的 rowid `IN` 过滤改为 Python 侧成员过滤（取更深再筛），
理由是实测发现 `IN` 的成本**按列表基数计价**：`IN(1)`=2.5 ms、`IN(500)`=3.8 ms、
`IN(1081)`=5.6 ms，而匹配不到任何行与无谓词同价（2.5 ms）。

健康库上它确实更快（5.13 → 3.19 ms，192 组组合逐一比对零差异），
但在「某用户的 active 事实只占向量表一小部分」的多用户库上**灾难性退化**：
为凑够 pool 必须反复全表扫描，实测 active=50/noise=2000 时
**4.21 ms → 56.94 ms（13.5 倍）**。改用「单次固定深度取深再筛」同样不行：
比旧实现更慢（4.98 vs 4.12 ms）**且结果不完整**。

根因：`IN` 虽按基数计价，但它把过滤**下推到同一次扫描内**；取深再筛把过滤
变成「多扫几遍」，成员稀疏时后者远贵于前者。那 1.9 ms 是 vec0 的既定成本，
不是可回收的浪费。**已完整还原 `retriever.py`，全量测试复绿。**

### Performance (读数路径：recall 冲突扫描惰性化)

**recall 的端到端延迟下降 33%，`conflicts` 契约不变。**

`api.recall` 每次都无条件调用 `_load_conflicts`，它要读该用户**全部** active 事实
并批量拉取它们的 scope 绑定，只为报告「同一单值键下的两个不同 object」。
实测在 1059 条事实的真实库上占 **6.4 ms**——是 `retriever.search`（12.4 ms）之外最大的
单笔开销，而写入路径的 `validator._check_conflict` 本就拦着这类行，所以正常库上它
每次都在为一个按设计不该发生的情况做全表扫描，产出恒为 `[]`。

现改为**门控惰性计算**：新增 `_has_conflict_candidate`，把行按 `(subject, predicate)`
分组但**不看 scope**，只要没有任何键持有 ≥2 个不同 object 就直接返回。

这一步成立的推理是关键：按 scope 切分只会在 `(subject, predicate)` 组内**再细分**，
两件共享 scope 集的事实必然共享该键，反之不成立——所以「不分 scope 也没有冲突」
可以**证明**「分 scope 后也没有冲突」，提前返回不会漏报。反之门控返回 true 只是
**不确定**（两个 object 可能分处不同 scope，那不算冲突），必须落到完整扫描再判。

实测（同库、`--repeat 30`）：

| 指标 | 前 | 后 |
|---|---|---|
| `api.recall` p50 | 23.56 ms | **15.71 ms** |
| `api.recall` p95 | 26.40 ms | **20.52 ms** |
| RPC 往返 p50 | 24.82 ms | **16.24 ms** |
| `_load_conflicts`（干净库） | 6.42 ms | **1.76 ms** |

`conflicts` 字段仍在响应里，语义不变——保留而非删除，因为它虽是文档化的公开 API 字段
（`docs/python-library.md`），却已核实 host 侧 `memory_recall` 只读 `r.facts` 不消费它；
惰性化拿到同等收益且零破坏性变更。

补 3 个门控测试（干净库短路 / 跨 scope 时必须下沉 / 多值键不武装门控），并做**双向验证**：
门控恒 `False` → 3 个测试失败（含 2 个原有冲突测试）；恒 `True` → 2 个新测试失败。
两个失败方向由不同测试捕获，证明门控两个方向都真正受约束。全量 `pytest` 600 passed。

**回归测试与 27 轮修复保持同一标准**：新增测试均先在旧实现上确认会失败。

### Fixed (第二十七轮：代码审查修复)

一次全库审查（含 Python 库与 dsh 插件两侧）后的成批修复。按「先修会影响正确性的」排序。

**B1 设置面板删除画像行是空操作**（`dsh/src/client/MemorySettingsSection.tsx`）

`saveEnvelope()` 在提交前 `filter(r => !r.deleted)` 把删除行滤掉了，于是删除按钮只改本地
状态、不写回库——用户看到行消失，重开面板它又回来了。现在删除行照常进入提交信封，`present`
（行数上限判断）仍只看未删除行。补回归测试，并**先在旧实现上验证过它会失败**。

**B2 注入防护两层不一致，且画像写入完全绕过清洗**

- `atom_memory/profile.py`：`_clean()` 原来只做 `" ".join(text.split())[:limit]`，即纯截断。
  画像值最终是模型可见文本，与事实正文适用同一条规则，但这条路径上 bidi 覆盖符（U+202E）、
  零宽字符原样入库。现改为走共享清洗器（`clean_field` / `clean_body`），行长上限与折叠语义不变。
- `atom_memory/api.py`：单行写入本已清洗，但用的是 `max_content_chars`（20000），进到
  `write_profile_rows` 又被截到 2000——先按一个数清洗、再按另一个数存储。现统一为导出的
  `MAX_PROFILE_VALUE_CHARS`，并把截断回报保留在能看到原文的那一层。
- `dsh/src/memory-data.ts`：宿主侧的 `isStripped` 是**手工维护的黑名单**，比 Python 侧的
  类别判定（`Cf`/`Cc`）弱——实测漏掉 41 个 `Cf` 码位（U+0600–0605、U+06DD、U+070F、
  U+0890–0891、U+08E2、U+110BD、U+110CD、U+13430–1343F、U+1BCA0–1BCA3、U+1D173–1D17A
  等）。这些字符不可见且非空白，会穿过空白折叠直达提示词。现以 `\p{Cf}|\p{Cc}` 兜底，
  显式表保留为「可读子集」，`U+200C/U+200D` 两个连接符继续豁免（与 Python 侧一致）。

**B3 写入路径的内容身份判定是 check-then-act，中间隔着真实的让步点**

`_persist_fact` 先查指纹、后 `INSERT`，但两者之间有一次
`await asyncio.to_thread(embed_func, …)`——事件循环在这个窗口里可以跑别的协程，于是同一
claim 的两个候选会双双看到「无既有行」并各插一行，留下两条完全相同的 active 事实：正是指纹
要防的东西，而且**任何修复流程都发现不了**（索引一致性检查比对的是 `facts` 与 FTS/向量索引，
不是 `facts` 与自身）。

修法是**在写事务内重检**，而不是加唯一索引——身份是「按 scope 窗口」计的，同一 claim 在第二个
项目里**刻意**要成为第二条事实（那条独立复述正是抽象提升的证据），
`(user_id, content_fingerprint)` 的部分唯一索引会禁掉设计所需的跨 scope 情形。补两个测试：
一个复现竞态（用第二个连接在 embed 回调里提交），一个确认跨 scope 复制行为不变；
竞态测试**已验证在去掉重检后会失败**。

**B3b 候选项终态与任务终态分两次提交**

失败路径上 `_finish_candidate` 先自己提交，`_record_failure` 再提交一次；两步之间崩溃会留下
「候选项已 `error`、任务仍 `running`」，重试于是重跑一个候选项已关闭的抽取，而
`_finish_candidate` 只写终态、那行再也转不出去。现改为同一事务。

**B4 容量归档的写入直到维护流程末尾才提交**

`enforce_capacity` 的 `UPDATE` 停在隐式事务里，而 `record_event` **无条件提交**——维护流程
后续任何一步写事件，都会把「做了一半的归档」刷到盘上，且 `facts_archived` 事件从未记录：
归档发生了，审计没有。现在归档自己提交；维护流程的 `except` 分支补 `_safe_rollback()`，
因为「吞掉异常」配上「事务还开着」正是局部写入变成永久写入的路径。

**B5 近重复探测的 `k` 是全局截断**

`facts_vec` 没有 owner 列，「同一使用者」只能在 join 之后判断，于是候选集是**全库**最近的
5 行——多用户库里这 5 行可能全属于别人，真正的重复排在第 6 位永远不被检查，去重静默失效且
**看不出来**（写入成功，只是看起来像一条新事实）。新增 `dedup_probe_k`（下限 50），
owner 过滤与其余判定一并留在 Python 侧。补测试：40 个其他使用者的等距事实挡在前面时，
改写副本仍能被折叠；**已验证旧的 `k = 5` 下该测试会失败**。

**文档**

- `dsh/README.md`：注入预算口径说明修正。`injectedSummaryTokens` 约束的是摘要正文，
  宿主加围栏（行前缀 + `BEGIN/END` 边界）实测**另需 55–134 tokens**，整个注入块会略高于该值。
- `dsh/src/config.ts`：`maxProfileRows` 注释原称画像「渲染进会话系统提示词」，不准确——
  它由 `memory_user_md` 工具提供，进入的是**对话**；冻结进系统提示词的是紧凑摘要。
- `docs/memory-semantics.md` / `docs/python-library.md`：补 `dedup_probe_k`、写事务内重检、
  以及「身份为何不能由数据库约束表达」。

**前端**：`modelManual` 单选状态原只在挂载时取一次初始值，设置重载或他人写入后可能与库中
实际值不一致；现按本文件既有的 props 同步模式，在库中出现新值时重新采纳。

### Cleaned (第二十七轮附：审查中记录的清理项)

- **`bridge.dispose()` 的「优雅退出」名不副实**（`dsh/src/bridge.ts`）。写 stop 帧后**下一行
  就 `kill()`**，SIGTERM 可能在子进程读到该帧之前就到——注释承诺的 flush 并未发生，未落盘的
  WAL 是常态而非偶发。现改为：发帧 + `stdin.end()`（子进程见到 EOF 也会走），给
  `GRACEFUL_STOP_MS = 2s` 让其自行退出，**只有到期仍未退的才 kill**（对已退出的进程 kill
  不仅无用，pid 被复用还有害）。Python 侧本就支持该语义（`rpc.py`：收到 `stop` 即 break 并
  `shutdown()` 落盘）。补两个测试：子进程自行退出时不 kill、超时则 kill；已验证旧实现下
  「stop 帧」测试会失败。
- **`Runtime` 类已成死代码**（`dsh/src/runtime.ts`）。设置层成为唯一数据源后，这个持有
  value、带订阅通知的类既无写入者也无订阅者——留着就是「实时值还可能从第二个地方来」的
  隐患。移除类及其 5 个测试；`controller.test.ts` 改用与生产一致的 `{ get() }` 字面量。
- **`remote.ts` 挂载了 4 个浏览器永不调用的描述符**（`getRuntime` / `health` /
  `overviewStatus` / `unarchive`）。描述符的作用是让浏览器能调；挂一个 UI 不调的方法只会
  扩大 stub 面，且每个都要与 Host 参数名保持同步（`assertExactArguments` 在挂载时校验，
  而参数名由 Host 方法源码文本推导）。现只挂 13 个实际调用的。Host 方法与 Python RPC
  全部保留。原测试断言「与 Host 标记集**完全相等**」，现改为「必须是 Host 标记集的**子集**」
  （描述符多于 Host 才是缺陷），并显式钉住这 4 个不被挂载。

### Not changed (第二十七轮附：一个刻意不修的点)

**变体选择符（U+FE02–FE0F、U+E0100–E01EF）不被剥离**——已实测确认，并在
`atom_memory/sanitize.py` 与 `tests/test_sanitize.py` 中写明理由，以免下次审查重报：

- 它们是 Unicode `Mn`（非间距标记）而非 `Cf`，所以类别兜底覆盖不到；显式表只列了
  VS-1/VS-2（U+FE00/FE01）。
- **无法用于隐藏指令**：变体选择符是前一个可见字符的**后缀修饰符**，只选择字形变体或强制
  emoji 呈现，不携带字母表；对比 tag 块（U+E0020–E007F，as-is 的不可见 ASCII 编码）已被
  整段剥离。所以「人看到 X、模型读到 Y」在这条路上走不通。
- 真实影响只是**字符串不等**（`"A"` 与 `"A\uFE0F"` 字节不同），属规范化问题而非注入问题。
- 剥掉 U+FE0F 会**破坏合法内容**（它是 `❤` 渲染成 `❤️` 的方式）。移除会改写用户写下的东西，
  保留只带来一个很窄的匹配边界情形——因此保留。
...
该块**下面还有没有别的分组**。块头概括的是整块（`[全局规则 · 2 条 · 决策规则 1 · 教训 1]`
同时声称两个分组），而合并后它只贴着第一个标签，于是渲染成：

```
| [全局规则 · 2 条 · 决策规则 1 · 教训 1] ## 决策规则   <- 块头声称两个，却只贴着第一个
| - 全局规则
| ## 教训                                             <- 悬空、失去归属
```

多分组块正是真实摘要的常态（每个存活的分组各出一个标签），所以这不是边角情形。原测试未发现，
因为它的 Python fixture 只造了单分组块 —— 断言"块头不会紧邻标签"在单分组下恒真，**空转通过**。

**为什么不是直接取消合并**：合并本身是对的（两行说的是同一块，各付一次 `| ` 前缀），错的是
**无条件**合并。单分组块下块头与标签确实等价，仍然合并省一行。

**dsh 侧**：

- `memory-data.ts`：新增 `hasSecondSection`（从 `index + 2` 起扫到块尾——下一个块头、分隔符
  `:` 或输入结束——判断是否还有第二个标签），`foldBlockHeadings` 以它作为合并的前提；抽出
  `isBlockHeading` 供两处共用，并新增 `BLOCK_SEPARATOR` 常量与 Python 侧
  `_BLOCK_SEPARATOR` 对齐。合并的 JSDoc 写明**为什么是有条件的**，不再宣称"只有这一种相邻
  关系会被合并"。
- 测试：`memory-data.test.ts` 新增 3 例（多分组块不合并、单分组块仍合并、同一摘要内多个块
  各自独立判定），并把跨语言契约测试的 fixture 参数化以生成多分组摘要 —— 新断言
  「合并过的块头后面不得再跟标签」在旧代码下**确实失败**（已 stash 源码验证：3 例红）。
- 文档：`README.md` / `README.zh.md` / `docs/scopes.md` 同步为条件化规则，`scopes.md` 的
  示例块（原先展示的正是这个 bug 的形态）改为正确渲染。

**兼容性**：纯渲染层改动，不改 Python 侧返回的 digest、不改协议、不改配置。多分组块每块多花
一行 `| ` 前缀的代价，换来块头不再与自己的正文矛盾——与 `_render_block_heading` 让条数取
"实际渲染行数"是同一个不变式。
...
- **注入分块**：紧凑摘要在有上下文时按块渲染（`[当前项目: api · N 条 · 类型分布]` /
  `[客户: acme …]` / `[阶段: draft …]` / `[全局规则 …]` / `[条件规则: k=v …]`），每块独立预算、
  全局块有保底预留；块头的条数是该块**实际渲染**的行数而非原始条数，因此不会与它下面的正文
  矛盾；总预算仍是**硬上限**：装不下时按"由具体到一般"的顺序回收（条件规则 → 阶段 → 远祖 →
  当前作用域 → 全局规则最后），再放弃页脚（唯一不承载记忆的一行），最后才丢整块。
  绑定在不可见作用域上的事实**根本不注入**（设计里的"其他项目参考默认不注入"，仍可用
  `memory_recall` 取到）。detail 深度刻意不分块（那是给人定位/编辑用的）。
- **行标记按注入后仍可区分来选**：宿主给每行加 `| ` 前缀（列零不变式），原文的首字符层级因此
  消失。分组标签由 `# ` 改为 `## `、块间空行改为单独一个 `:` 行、页脚去掉"N 条事实 · 类型分布"
  前缀改为 `-- （按行）…`，使加前缀后四类行各自有不同的首字符（`[` / `#` / `-` / `:`）。
  含路径的值改为保留尾部截断（`C:\Users\fuqia\.dsh\profiles\web\node_modules` →
  `C:…web\node_modules`），正文仍从头部截断。净 token 影响约 ±0（实测同一份库 771 → 773）。
- **块头与分组标签合并**（`memory-data.ts` 的 `foldBlockHeadings`）：Python 侧返回的块头
  （`[当前项目: api · N 条 · 类型分布]`）与紧随其后的分组标签（`## 决策规则`）说的是同一件事，
  各自却要付一次 `| ` 前缀的代价。注入时合并为一行（`[当前项目: api · N 条] ## 决策规则`），
  实测同一份库 41 → 38 行。只合并这一种相邻关系：块头后面直接跟条目或分隔符时原样保留。
...
- **画像是一张独立的持久表**，条目只在两种情况下进入：用户手动添加，或采纳「生成画像」的推荐；只在你删除时离开。删除是**永久**的（没有任何东西再重建它），记忆库里的原子事实**不受影响**——属性仍会被召回、仍进摘要，只是不再作为画像条目。
- **「生成画像」由用户手动触发**，调用链跨两侧：Python 侧 `profile_candidates` 给出可填充槽位（原投影规则复用为**建议来源**）与已有 `(section, key)`，**排除已收录**；dsh 侧的模型（`profile-synthesis.ts`，复用抽取模型的配置，走 `buildLlmCompleter` 抽出的同一模型解析 + 自定义端点 + 截断检测骨架）把它们提炼成推荐并做去重、排除与上限裁剪；用户在弹窗里勾选后，`write_profile` **一次性整批**写入——批内要么全成、要么全不动（上限在写入前对"批次后的表大小"校验）。生成过程**不写任何东西**。
- **条目上限**：新增 `MemConfig.max_profile_rows` / 配置 `maxProfileRows`（默认 50，0 = 不限）。画像会经 `user_md` 渲染进系统提示词，**每一条都在每个请求上付费**，这是那笔开销的硬上限。超限时**拒绝新增**并回报 `{current}/{limit}`（改动已有条目仍允许，删一条即释放一个槽位），不做"静默淘汰最旧一条"。渲染另有独立预算（`user_md_token_limit`），截断时**明示**"另有 N 条未展示"，不再看起来像完整列表。
- **`pinned`（固定）列删除**（迁移 010）：它冻结的是"自动写入"，而自动写入已不存在；`worker.py` 中"固定画像行保护对应事实不被归档"随之移除（那曾是 `pinned` 唯一的行为差异）。画像行新增/保留 `source` 语义改为**来源标记**（`user` = 手动，`generated` = 采纳的推荐），并在面板里显示。
- **迁移 010 清空旧画像行**（这是用户明确选择的"清空重来"）：旧行是投影输出，用户的存留意图只是"事实推导出了这些"，不是一份精心维护的清单；**事实本身不动**，第一次「生成画像」可以把它们重新推荐回来。迁移 009 为空占位——其版本号被一个发布前已回滚的设计（`profile_deletions` tombstone）占用，而 `_apply_migrations` 逐号读取，编号不能留空洞。
- **备份携带画像的 `source`**，恢复时原样往返，不会把用户手写的条目重新标记成"生成"的；`pinned` 不再往返。

**修复过程中发现并修掉的一个自造 bug**：重写客户端 `saveAllProfile` 为单批调用时漏了 `unwrap`，于是 Host 返回的 `{ok:false}`（例如超限拒绝）会被当成**保存成功**、弹窗照常关闭、用户看不到任何原因。现在 `unwrap` 结果、失败存入 `lastError` **并抛出**，面板捕获后**保持编辑器打开**并显示原因——拒绝不能表现为成功。

**测试**：`tests/test_ui_api.py`（事实不再产生画像行、删除永久有效、条目上限拒绝新增但允许编辑、批次全或无、备份往返含 source）、`tests/test_lifecycle.py`（学一条事实不碰画像）、`tests/test_retriever.py`（候选沿用旧投影映射、渲染截断明示）、`tests/test_db.py`（迁移链：v3/v1 老库升到 v10 后无 `pinned` 列、旧画像行被清空、事实存活）、`dsh/tests/`（批次单次调用、拒绝时抛出、生成结果归一化与异常形状、UI 生成→勾选→加入→保存、超限报错保持弹窗、无 pin 列）。
