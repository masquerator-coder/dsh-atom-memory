...
| `maxVectorDistance` | `0.70` | 语义召回的距离上限（余弦距离，部署期）。这是唯一能表达「都不相关」的信号——纯排名做不到，因为二十条烂结果里最近的那条照样排第一。按 `BAAI/bge-small-zh-v1.5` 实测标定：相关对 0.33–0.54、跨语言相关约 0.65、无关 0.67–0.85。单语言库想更宽松可降到约 0.60；`0` 或留空即不发送该参数，等同于关闭闸门 |
| `minRelevance` | `0.15` | 融合相关度地板（0..1，部署期）。低于它的候选**根本不返回**，因此「记忆里没有相关的」是可表达的。默认刻意非零：取 0 时地板永不触发，召回永远返回满 `maxRecalledFacts` 条，大库与小库在结果上无从区分。**实测定位是安全网而非主力**——返回的 8 条里最弱一条在域内查询得 0.78–0.86、域外查询 0.39–0.42，所以它要到约 0.45 才开始裁剪、到 0.60 会让合法的罕见查询直接返回空。想激进裁剪可自行调高，以上数值即标定依据 |
| `candidatePoolMultiplier` | `4` | 融合前每条检索腿往下看的深度倍数（正整数，部署期）。两条腿各取 `maxRecalledFacts × 该倍数` 条再融合，**重排之后**才裁剪回 `maxRecalledFacts` 条。RRF 奖励的是两个榜单的*一致性*：同时进两榜得 `2/(k+1)`，只进一榜最多得 `1/(k+1)`，所以**同时排在两榜 top-k 之外的事实无论多重要都不可能被融合进来**。实测 617 条事实的库里，真正回答某查询的那条词法排第 15、语义排第 11，1x 时不可达、4x 时可召回。`1` 恢复旧行为 |
| `recencyHalfLifeDays` | `30` | 重排中「时间新近」的**半衰期**（天，部署期）。与 `reinforce` 侧的强度衰减是两件事：这一项决定「新近值多少分」，那一项决定「复用值多少分」，故意独立——可以既要强度衰减慢、又要时间衰减快。年龄按 `last_used_at` 计（用过就按最后一次使用算），所以长期在用的事实不会因为「创建得早」被淘汰。实测同一结果集内的 recency 跨度：半衰期 1 天 → 0.385（最新使用的那条升到第 1）；30 天 → 0.131（relevance 主导）；365 天 → 0.011（时间项几乎关闭）。库内事实周转快就调小，耐久知识就调大 |
| `recencyReferenceWindowDays` | `0`（= 派生 3×半衰期） | recency 相对位移的**封顶**（天，部署期）。它不是独立参数而是与半衰期的比例：默认 `None`/`0` 时派生为 `3 × recencyHalfLifeDays`，所以只调半衰期时会自动跟随，封顶不会悄悄掉到它本该保护的跨度之下。封顶过低会把不同年龄压成同一个 credit（`window == 半衰期` 时所有超过一个半衰期的候选都变成 0.5），一般**不需要单独改这一项** |
