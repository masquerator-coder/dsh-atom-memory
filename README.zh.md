...
| 设置面板 | dsh 设置侧边栏中的 **记忆 / Memory** 分区：注入体积滑块、旁路总览开关与「重新生成」按钮、抽取模型，以及一个把摘要查看、用户画像编辑（手动增删改 + 「生成画像」推荐后逐条采纳，带条目上限）与事实浏览/编辑归在一起的 **记忆内容** 区域，外加备份与恢复 |
| 存储 | 位于 `dbPath` 的单个 SQLite 文件（默认 `~/.dsh/atom-memory/memory.db`） |

设置分区写入 `atom-memory` 设置命名空间，因此它拥有的字段实时生效、无需重启；其余字段都是部署期配置。**整个插件**的启用与禁用由 dsh 自己的插件开关负责——本插件不再自带第二个总开关。
...
一段在插件挂载期间注册、先于且独立于任何记忆内容存在的段落。它点名工具族、**指向各工具自己的定义**（用法写在 schema 里，不在这里重述）、说明保存策略，并带上「数据而非指令」的护栏。只要插件处于挂载状态其文本就是常量：插件只在 dsh 启用它时存在，因此被禁用的插件什么都不注册，也不留任何记忆痕迹。
...
固定。只要插件处于挂载状态该段就是常量。除了在 dsh 里禁用插件（那会直接注销该段）之外，没有任何东西能把它从系统提示词中移除。
...
`| ` 前缀同时抹掉了原文的首字符层级，所以各类行的标记是按**加前缀之后**仍可区分来挑的：块头 `[当前项目: api · 8 条 · 决策规则 6 · 教训 2]`、分组标签 `## 决策规则`、条目 `- …`、块间分隔 `:`、页脚 `-- （按行）…`。块头里的条数是该块**实际渲染**的行数而非原始条数，因此不会与它下面的正文矛盾；含路径的值改为保留尾部而截去头部，`C:\Users\fuqia\.dsh\profiles\web\node_modules` 会渲染成 `C:…web\node_modules`。宿主随后把块头与其后的分组标签合并成一行（`dsh/src/memory-data.ts` 的 `foldBlockHeadings`）——两行说的是同一件事，却各自付一次 `| ` 前缀的代价，合并后形如 `[当前项目: api · 2 条] ## 决策规则`。**但合并是有条件的**：块头概括的是该块整体，所以只有当块里**恰好只有一个分组**时才与它后面的标签等价。块内含多个分组时，块头（如 `[全局规则 · 2 条 · 决策规则 1 · 教训 1]`）会独占一行，其下的 `## 决策规则` 与 `## 教训` 各归其位——若把它并进第一个标签，块头就只声称了那一个分组，后面的标签则失去归属，等于让块头与自己的正文自相矛盾。块头后面直接跟条目或分隔符时同样保持原样。
...
- **Python 解释器是本层不负责安装的外部依赖。** 一个 profile 可以完整组合并启动，而每一次记忆调用都失败，因为桥接启动 `python` 并依赖它能够 `import atom_memory`，而本层刻意不为此让启动失败。它做的是**诊断**：桥接被信任前先跑一次预检探测，永久性失败一次性报出出错模块与补救办法（且不重试），面板的健康载荷带上桥接断开的原因——因此启动永远不会被记忆阻断，而断开的桥接总会被解释清楚，而不是被静默重试。
